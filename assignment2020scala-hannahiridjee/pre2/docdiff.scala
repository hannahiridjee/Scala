...
def occurrences(xs: List[String]): Map[String, Int] = { xs.distinct.map(x => (x,xs.count(_ == x))).toMap }
...
def prod(lst1: List[String], lst2: List[String]) : Int = {
    ...
    val newList = for((k,v)<-map1) yield 
        if(map2.contains(k)) v * map2(k) 
        else 0

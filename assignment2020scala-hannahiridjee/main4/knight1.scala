...
def count_tours(dim: Int, path: Path) : Int = {
    if(path.length == dim * dim) 1 else (for(i <- legal_moves(dim, path, path.head)) yield count_tours(dim, i:: path)).sum
    
}

def enum_tours(dim: Int, path: Path) : List[Path] = {
    if(path.length == dim * dim) List(path) else (for(a <- legal_moves(dim, path, path.head)) yield enum_tours(dim, a:: path)).flatten
...
def first_tour(dim: Int, path: Path) : Option[Path] = {
  if(path.length == dim * dim) Option(path) else {
      val lstPos = legal_moves(dim,path,path.head)
      val func = (p:Pos) => first_tour(dim,p::path)
      first(lstPos,func)
  }
}

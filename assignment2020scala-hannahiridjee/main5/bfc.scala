...
def jumpRight(prog: String, pc: Int, level: Int) : Int = {
    if((pc > prog.length-2) ||(prog(pc)== ']' && level == 0)) pc + 1
        ...
        prog(pc) match {
            ...
            case ']' => if(level != 0 )jumpRight(prog, pc + 1, level - 1) else jumpRight(prog, pc + 1, level) 
...
def jumpLeft(prog: String, pc: Int, level: Int) : Int = {
     if(level == 0 && prog(pc) == '[') pc + 1 
        ...
        prog(pc) match {
            case '[' => if(level!=0 && pc==0) -1 else if(level !=0) jumpLeft(prog, pc - 1, level - 1) else jumpLeft(prog, pc - 1, level)
            case ']' => jumpLeft(prog, pc - 1, level + 1)
            case _ => if(level!=0 && pc ==0) -1 else jumpLeft(prog, pc - 1, level)
      }  
     }
}
...
def jtable(pg: String) : Map[Int, Int] = {
  val idx = (0 to pg.length-1).toList
  val lst = for(i <- idx) yield{
    pg(i) match{
      case '[' => (i,jumpRight(pg,i+1,0).toInt)
      case ']' => (i,jumpLeft(pg,i-1,0).toInt) 
      case _ => (i,-10)
    }
  }
  val lst2 = lst.filter(_._2 != -10)
  val m = lst2.toMap
  m
...
def compute2(pg: String, tb: Map[Int, Int], pc: Int, mp: Int, mem: Mem) : Mem = {
    if(pc>pg.length-1) mem
        ...
        pg(pc) match{
            ...
            case '-' => compute2(pg,tb,pc+1,mp,write(mem,mp,sread(mem,mp)-1)) 
            case '.' => println(sread(mem, mp).toChar) 
                        compute2(pg,tb,pc+1,mp,mem)

            case '[' => if(sread(mem, mp) == 0) compute2(pg,tb,tb.get(pc).get,mp,mem) else compute2(pg,tb,pc+1,mp,mem)
            case ']' => if(sread(mem, mp) != 0) compute2(pg,tb,tb.get(pc).get,mp,mem) else compute2(pg,tb,pc+1,mp,mem)
            ...
            case '#' => println(sread(mem, mp).toInt)
                        compute2(pg,tb,pc+1,mp,mem)
...
def compute3(pg: String, tb: Map[Int, Int], pc: Int, mp: Int, mem: Mem) : Mem = {
    ...
    if(pc>pg.length-1) mem
        ...
        pg(pc) match{
            ...
            case '[' => if(sread(mem, mp) == 0) compute3(pg,tb,tb.get(pc).get,mp,mem) else compute3(pg,tb,pc+1,mp,mem)
            case ']' => if(sread(mem, mp) != 0) compute3(pg,tb,tb.get(pc).get,mp,mem) else compute3(pg,tb,pc+1,mp,mem)
...
def run3(pg: String, m: Mem = Map()) = {
    val pgOpt = optimise(pg)
    compute3(pgOpt,jtable(pg),0,0,m)
}

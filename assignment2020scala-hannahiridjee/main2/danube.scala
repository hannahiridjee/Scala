...
def get_csv_url(url: String) : List[String] = {
    val file = Try(Source.fromURL(url)("ISO-8859-1").getLines().toList.drop(1)).getOrElse(Nil)
    file
}
...
def process_ratings(lines: List[String]) : List[(String, String)] = {
    ...
    val lst= for(i <- lines) yield{
        if(i.split(",").takeRight(1)(0).toInt >= 4 ) {
           //Some((i.split(",").take(1)(0),i.split(",").take(2).takeRight(1)(0)).toString)
           Some(i.split(",").take(1)(0).toString , i.split(",").take(2).takeRight(1)(0).toString)
        }
        else None
    }
    lst.flatten
    /*for(i <- (0 to lines.size-1).toList) yield{
        val tl = lines(i)
        val idx = tl.length -1
        val last = 
        lines.filter(tl(idx).toInt >= 4)
    }
    lines*/
...
def process_movies(lines: List[String]) : List[(String, String)] = {
    ...
    val lst: List[(String,String)] = for(line <-lines) yield{
        //((line.split(",").take(1).toString) , (line.split(",").take(1).toString))
...
def groupById(ratings: List[(String, String)], 
              ...
              ratings match{
                  ...
                  case x::lst => 
                    val key = x._1      
                    val value = x._2   
                    val existing = m.find(_._1 == key)
                    if(existing != None){
                        //there is
                        val oldValue = m.get(key).get
                        val oldLst = value.toString :: oldValue
                        groupById(lst, m.updated(key,oldLst))
                    }else{
                        groupById(lst, m.updated(key,List(value.toString)))
                    }
              }
...
def favourites(m: Map[String, List[String]], mov: String) : List[List[String]] = {
    val lst = for((k,v)<-m) yield {
        if(v.contains(mov)){
            v.filter(_ != mov)
        }
        else null
    }
    lst.toList.filter(_ != null)
...
def suggestions(recs: Map[String, List[String]], 
                mov_name: String) : List[String] = {
                    ...
                    val favs2 = favs.flatten //res1
                    val favs3 = favs2.groupBy(identity).view.mapValues(_.size).toMap.toList.sortBy(-_._2)
                    val lstRet = for(v <- favs3) yield {
                        v._1
                    }
                    lstRet
                }
...
def recommendations(recs: Map[String, List[String]],
                    ...
                    mov_name: String) : List[String] = {
                        val sug = suggestions(recs,mov_name) //list(str) of movieIDs
                        val sug2 = sug.take(2)
                        val lst = for(s<-sug2) yield movs.get(s)
                        lst.flatten
                    }
...
def most_recommended(recs: Map[String, List[String]],
                     movs: Map[String, String]) : List[(String, Int)] = {
                         val lstRecs = for((k,v)<-movs) yield{
                             recommendations(recs,movs,k)
                         }
                         val lstRecs2 = lstRecs.flatten
                         val lstRecs3 = lstRecs2.groupBy(identity).view.mapValues(_.size).toMap.toList.sortBy(-_._2)
                         lstRecs3
                     }
